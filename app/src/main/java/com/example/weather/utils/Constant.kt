package com.example.weather.utils

object Constant {
    const val BASE_URL = "http://dataservice.accuweather.com/"

    const val API_Key:String ="p9RZ7cqPp1AYYKc5b6kXR3NFiZr3kKeV"
    const val API_Key1:String ="JpV3XPBHu3X4Jy12xXHI9IfePT4TLcUn"
    const val API_Key2:String ="LJwc0mW9keAgXPP6e3F8Z2kloKBCuluC"
    const val API_Key3:String ="0q9k1wAVUFAQUkASbjvYRfG65YSy0wzK"
    const val API_Key4:String ="HunVnutifMYDc4Choxx7jJhPtAJ79Buj"
    const val API_Key5:String ="ZD7lTPMiGgguddpGrgsEcvhUwDIb5e8S"
    const val API_Key6:String ="6P2SvsAnCe1FStBLmyBOcA6lXHCZtI7k"
    const val API_Key7:String ="QdgMQ5Ditlh0IWvSLj5AW09WBP24lUrX"
    const val API_Key8:String ="n5SqWU3R7krVeTrIbI5qiaTilDNDmG6u"
    const val API_Key9:String ="BS3zmvLPGQ2PyTjOqONW7UfXJNJJ720n"
    const val API_Key10:String ="rUOGzHqVFYaEC9QkXw48o6IeiSGUGi8r"
    const val API_Key11:String ="3ZoO3gRBQVgGJjXCPWTdCFbQZ746GNtE"
    const val API_Key12:String ="0Kw77wDCpvE2GkhPgd6ZG2XkhmwhN4md"
    const val API_Key13:String ="Fc1Clk9yGGzwqbvnGiRbeHs7E8hXE8FF"
    const val API_Key14:String ="B7qyEGHU9bWmGPOKKy5DuCAi0ndMlhcc"
    const val API_Key15:String ="My9KEfGPTI2ShIyrzRAoWLUum4Uz4K7w"
    const val API_Key16:String ="CSNmaE7A4wvzjiG3gkUmJg7YtuZNp8HS"
    const val API_Key17:String ="tefIbnvi5t45hVjHDvlHCxJJ5GCMdjc1"
    const val API_Key18:String ="57GAkJKZmmsWLvD3ARSITBqes987JdSE"
    const val API_Key19:String ="Jv90yxeRPYAMvSiMuWAocvmTxJnDkJWw"
    const val API_Key20:String ="srFGLkGA446Sr1Qvf2KL94gCD9kbUp1Y"
    const val API_Key21:String ="XpnYSVLeoGwZsX7AH3IpcN6vApWCSuDw"
}